...
<div class="relative pt-32 pb-20 lg:pt-40 lg:pb-28 overflow-hidden bg-white">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <h1 class="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight mb-6">
            Istirahat Nyaman di <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Yogyakarta</span>
        </h1>
        ...
        <form action="{{ route('hotel') }}" method="GET">
            <div class="bg-white/95 backdrop-blur-md p-3 md:p-2 rounded-2xl md:rounded-full flex flex-col md:flex-row w-full max-w-2xl mx-auto shadow-2xl items-center border border-gray-200">
                ...
                <button class="w-full md:w-auto mt-3 md:mt-0 bg-primary hover:bg-secondary text-white px-8 py-3 rounded-xl md:rounded-full font-bold transition-all transform hover:scale-105 shadow-md">
                    Cari
                </button>
            </div>
        </form>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div class="group relative bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
            <!-- Image -->
            <div class="relative h-64 overflow-hidden">
                <img src="{{ $hotel['image'] }}" alt="{{ $hotel['name'] }}" class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700">
            </div>

            <!-- Content -->
            <div class="p-6">
                <div class="flex items-center text-gray-500 text-xs font-bold mb-2 uppercase tracking-wider">
                    <i class="bi bi-geo-alt mr-1"></i> {{ $hotel['loc'] }}
                </div>
                ...
                <div class="flex items-end justify-between border-t border-gray-100 pt-4">
                    <div>
                        ...
                        <p class="text-lg font-bold text-primary">{{ $hotel['price'] }} <span class="text-xs text-gray-400 font-normal">/ malam</span></p>
                    </div>

                    <a href="{{ route('hotel.detail', $hotel['id']) }}" class="bg-primary hover:bg-secondary text-white px-4 py-2 rounded-xl text-xs font-bold transition-all transform hover:scale-105">
                        Lihat Detail
                    </a>
                </div>
            </div>
